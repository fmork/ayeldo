...
const LogoutPage: FC = () => {
  ...
  useEffect(() => {
    const performLogout = async (): Promise<void> => {
      try {
        console.info('Initiating logout process');
        await logout().unwrap();
        // Refresh session state to reflect logout
    ...
    // Only attempt logout if user is logged in or session is undefined (to be safe)
    ...
    <Container maxWidth="sm">
      <Box sx={{ mt: 4, textAlign: 'center' }}>
        ...
        {session?.loggedIn === false && (
          <Typography>You are already signed out. Redirecting...</Typography>
        )}
      </Box>
    </Container>
  );
