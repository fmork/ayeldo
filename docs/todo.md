...
- ☐ Minimal Next.js app talking only to BFF
- ☐ Pages:
  - ☐ `/login`
  - ☐ `/albums/:id`
  - ☐ `/cart`
  - ☐ `/checkout/result`
- ☐ Fetch wrapper with credentials + CSRF header

Codex prompt: Scaffold Next.js pages that call BFF endpoints with credentials: include cookies and send CSRF header.
