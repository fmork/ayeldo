...
- ✅ Tenant membership repo with shared GSI lookups (user & tenant views)
...
## 9) HTTP API (formerly BFF) Auth & Session
...
- ✅ HTTP API (formerly BFF) → API auth (short-lived signed JWT)
...
- ✅ Creator albums management page with creation workflow
...
- [x] Extend image DTO/entity/repository marshalling to capture original key, variants, and processed timestamps.
