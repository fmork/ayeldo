  ...
  public initialize(): HttpRouter {
    ...
    this.addGet('/auth/authorize-url', async (req, res) => {
      await this.performRequest(
        async () => {
          const redirectRaw = (req as any).query?.redirect as string | undefined;
          let redirectTarget = redirectRaw;
          if (typeof redirectRaw === 'string' && redirectRaw.length > 0) {
            try {
              // If it is percent-encoded, decode it
              const decoded = decodeURIComponent(redirectRaw);
              redirectTarget = decoded || redirectRaw;
            } catch {
    ...
    this.addGet('/auth/callback', async (req, res) => {
      try {
        ...
        (res as any).redirect?.(target) ??
          (res as any).status(302)?.setHeader?.('Location', target)?.end?.();
      } catch (err) {
        const errorObj = err instanceof Error ? err : new Error(String(err));
        this.logWriter.error(`Callback handling failed`, errorObj);
        (res as any).status(400)?.json?.({ error: 'invalid_request' });
      }
    });
