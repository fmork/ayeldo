...
describe('media handlers with policy guard', () => {
  ...
  test('getAlbum allows public access', async () => {
    ...
    const out = await getAlbum({ tenantId: 't1', albumId: 'alb1', access: { mode: 'public' } as any }, deps);
    expect(out?.id).toBe('alb1');
  });

  test('getAlbum denies hidden without token', async () => {
    const deps = { albumRepo: { getById: async () => ({}) }, policy } as any;
    await expect(getAlbum({ tenantId: 't1', albumId: 'a', access: { mode: 'hidden' } as any }, deps)).rejects.toThrow('Forbidden');
  });

  test('getAlbum allows hidden with token', async () => {
    const album = { id: 'a', tenantId: 't', title: 'x', createdAt: new Date().toISOString() } as any;
    const deps = { albumRepo: { getById: async () => album }, policy } as any;
    const out = await getAlbum({ tenantId: 't', albumId: 'a', access: { mode: 'hidden', linkToken: 'tok' } as any }, deps);
  ...
  test('listAlbumImages denies restricted when not member', async () => {
    ...
    await expect(
      listAlbumImages({ tenantId: 't', albumId: 'a', access: { mode: 'restricted', isMember: false } as any }, deps),
  ...
  test('listAlbumImages allows restricted when member', async () => {
    ...
    const out = await listAlbumImages({ tenantId: 't', albumId: 'a', access: { mode: 'restricted', isMember: true } as any }, deps);
