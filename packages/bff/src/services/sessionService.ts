...
export class SessionService {
  ...
  public createLoginState(): { state: string; nonce: string; codeVerifier: string; codeChallenge: string } {
    ...
    this.states.putState({ state, nonce, codeVerifier, createdAt: new Date().toISOString(), ttl }).catch(() => undefined);
  ...
  public async completeLogin(
  ...
  ): Promise<{ sid: string; csrf: string; profile: { sub: string; email?: string; name?: string } }> {
    ...
    const tokens = await oidc.exchangeCode({ code: params.code, state: params.state, nonce: st.nonce, codeVerifier: st.codeVerifier });
