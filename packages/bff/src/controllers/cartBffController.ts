...
export class CartBffController extends PublicController {
  ...
  public initialize(): HttpRouter {
    const requireSession = (handler: (req: any, res: any) => Promise<void>) => async (req: any, res: any) => {
    ...
    this.addGet('/bff/carts/:tenantId/:cartId', requireSession(async (req, res) => {
      const params = z.object({ tenantId: z.string().min(1), cartId: z.string().min(1) }).parse((req as any).params);
      const headers = this.makeAuthHeader((req as any).session?.sub, undefined, (req as any).session?.roles);
      const cartReq: any = { url: `${this.apiBaseUrl}/tenants/${params.tenantId}/carts/${params.cartId}` };
      if (headers) cartReq.headers = headers;
      const cartResp = await this.httpClient.get(cartReq);
      const priceReq: any = { url: `${this.apiBaseUrl}/tenants/${params.tenantId}/carts/${params.cartId}/price`, body: {} };
    ...
    this.addPost(
      ...
      requireSession(requireCsrf(async (req, res) => {
        const params = z.object({ tenantId: z.string().min(1), cartId: z.string().min(1) }).parse((req as any).params);
        const body = z
          .object({ imageId: z.string().min(1), sku: z.string().min(1), quantity: z.number().int().positive() })
          .parse((req as any).body);
        const headers = this.makeAuthHeader((req as any).session?.sub, undefined, (req as any).session?.roles);
        const addReq: any = { url: `${this.apiBaseUrl}/tenants/${params.tenantId}/carts/${params.cartId}/items`, body };
    ...
    this.addDelete(
      ...
      requireSession(requireCsrf(async (req, res) => {
        ...
        const headers = this.makeAuthHeader((req as any).session?.sub, undefined, (req as any).session?.roles);
        const delReq: any = { url: `${this.apiBaseUrl}/tenants/${params.tenantId}/carts/${params.cartId}/items`, body };
  ...
  private makeAuthHeader(sub?: string, tenantId?: string, roles?: readonly string[]): Record<string, string> | undefined {
    try {
